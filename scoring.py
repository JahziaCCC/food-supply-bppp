...
def compute_risk_index(
    ...
    """
    مؤشر موحد شفاف 0-100:
    - ضغط السلع (حتى 60)
    - ضغط النقل (حتى 20)
    - حرارة المخاطر في الأخبار (حتى 20)
    """
    ...
    if pct_values:
        ...
        commodity_score = clamp((avg_pct / 10.0) * 40.0)   # حتى 40
        spike_score = clamp((max_pct / 10.0) * 20.0)       # حتى 20
    ...
    # 3) حرارة الأخبار (0-20)
    geo_score = clamp((geo_heat / 100.0) * 20.0)
    drivers.append(f"حرارة المخاطر في الأخبار: {geo_score}/20")
    if geo_heat >= 60:
        triggers.append("ارتفاع حرارة المخاطر في الأخبار")
